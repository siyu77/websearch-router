...
// Root README + LICENSE ride along in the tarball: npm renders README.md on the
// package page and expects a license file next to it. Without them the npm page
// shows "This package does not have a README." and skips the license.
cpSync(join(root, 'README.md'), join(distRoot, 'README.md'));
if (existsSync(join(root, 'LICENSE'))) {
  cpSync(join(root, 'LICENSE'), join(distRoot, 'LICENSE'));
}
...
const publishedPkg = {
  ...
  // Single source of truth: the tarball/registry version follows the server
  // package.json (which src/version.ts also reads for /health & /mcp). Bumping
  // the release = editing packages/server/package.json's "version" in one place.
